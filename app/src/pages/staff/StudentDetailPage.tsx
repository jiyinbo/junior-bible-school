...
import { StudentSectionDialogs, type StudentEditSection } from './StudentSectionDialogs';
import {
  formatStudentBool,
  formatStudentField,
...
export function StudentDetailPage() {
  ...
  const { student, error, setError, load, setStudent } = useStudentDetail(studentId);
  const [editSection, setEditSection] = useState<StudentEditSection | null>(null);
  ...
  useEffect(() => {
    ...
    apiJson<{ data: { levels: TierOption[] } }>(`/api/v1/admin/sessions/${student.session.id}`)
      .then((response) => setTiers(response.data.levels.map((level) => ({ id: level.id, name: level.name }))))
      .catch(() => setTiers([]));
  }, [student]);
  ...
  const tierStatus = studentTierStatusDisplay(progress.level_completed, progress.programme_phase);
    ...
    <>
      ...
      <Grid container spacing={2}>
        <Grid size={{ xs: 12, md: 6 }}>
          <EditableDetailSection title="Registration" onEdit={() => setEditSection('registration')}>
            ...
            <FieldRow
              label="Status"
              value={<Chip size="small" label={tierStatus.label} color={tierStatus.color} />}
            />
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <EditableDetailSection title="Contact" onEdit={() => setEditSection('contact')}>
            <FieldRow label="Email" value={student.email} />
            <FieldRow label="Phone" value={formatStudentField(student.phone)} />
          </EditableDetailSection>
        </Grid>
        <Grid size={{ xs: 12, md: 6 }}>
          <EditableDetailSection title="Guardian" onEdit={() => setEditSection('guardian')}>
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <EditableDetailSection title="Personal" onEdit={() => setEditSection('personal')}>
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <EditableDetailSection title="Faith & church" onEdit={() => setEditSection('faith')}>
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <EditableDetailSection title="School & medical" onEdit={() => setEditSection('school')}>
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <EditableDetailSection title="Next of kin" onEdit={() => setEditSection('nextOfKin')}>
        ...
        {isAdmin && (
          <>
            <Grid size={{ xs: 12, md: 6 }}>
              <EditableDetailSection title="Student portal" onEdit={() => setEditSection('portalPin')}>
                <Typography variant="body2" color="text.secondary">
                  Reset the student&apos;s 4-digit portal PIN if they cannot sign in to take tests or view progress.
                </Typography>
              </EditableDetailSection>
            </Grid>
            <Grid size={{ xs: 12, md: 6 }}>
